...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async (isSilent = false) => {
    try {
      ...
      // 2. Fetch today's and monthly attendance bounds, plus pending overrides
      ...
      if (overrideRes.ok) {
        ...
        setPendingOverrides(overrideData.requests || []);
      }
  ...
  const resolveOverride = async (requestId: string, status: 'APPROVED' | 'REJECTED') => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Failed to resolve override request');
      }
  ...
  if (loading) return <LoadingState message="Assembling your system-wide admin dashboard..." />;
  ...
  const statCards = [
    ...
    { 
      ...
      subtext: 'Awaiting admin approval review',
      icon: AlertCircle, 
      color: 'bg-rose-50/50 border-rose-100 text-rose-900 icon-bg-rose-500' 
    },
    { 
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-200 p-6 rounded-2xl shadow-xs">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Full system overview — you have unrestricted administrator privileges</p>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className={`w-10 h-10 rounded-xl text-white shadow-md flex items-center justify-center flex-shrink-0 ${
                ...
                card.color.includes('icon-bg-rose-500') ? 'bg-rose-500 shadow-rose-500/20' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Widget 3: Pending Overrides Action Center */}
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-4 mb-1 pb-1">
              <h3 className="text-base font-bold text-slate-800">Pending Overrides</h3>
              <span className="px-2 py-1 bg-rose-50 text-rose-600 border border-rose-200 font-extrabold text-[9px] uppercase tracking-wider rounded-lg">
                Action Required
              </span>
            </div>
            <p className="text-xs text-slate-400 mb-5">
              Review and resolve employee late arrival override requests directly.
            </p>
            ...
            {pendingOverrides.length === 0 ? (
              <div className="py-12 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50/30 flex flex-col items-center justify-center my-auto min-h-[220px]">
                ...
                <p className="font-bold text-slate-700 text-xs">All Requests Resolved</p>
                <p className="text-[10px] text-slate-400 mt-1 max-w-[200px] mx-auto leading-relaxed">
                  No late arrival override requests are currently pending review.
                </p>
              </div>
            ) : (
              <div className="max-h-[320px] overflow-y-auto pr-1 space-y-3 custom-scrollbar">
                {pendingOverrides.slice(0, 5).map((req) => {
                  const emp = req.employeeId;
                  const empName = emp?.name || 'Unknown';
                  const empDept = emp?.department || '—';
                  const reqDateStr = new Date(req.date).toLocaleDateString('en-IN', {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric'
                  });
                  ...
                  return (
                    <div 
                      key={req._id} 
                      className="p-3.5 rounded-xl border border-slate-100 hover:border-indigo-100 bg-slate-50/30 hover:bg-indigo-50/5 flex flex-col justify-between gap-2.5 transition duration-150"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="font-semibold text-slate-800 text-xs">{empName}</div>
                          <div className="text-[10px] text-slate-400 mt-0.5">{empDept} • Late on {reqDateStr}</div>
                        </div>
                      </div>

                      <div 
                        onClick={() => toggleRequestExpansion(req._id)}
                        className="text-[10px] text-slate-700 bg-white p-2.5 rounded-lg border border-slate-200/60 hover:border-indigo-200 hover:bg-slate-50/50 leading-relaxed italic cursor-pointer transition flex items-start justify-between gap-2.5 group select-none"
                        title={expandedRequests[req._id] ? "Click to collapse" : "Click to expand"}
                      >
                        <span className={`flex-1 transition-all duration-200 ${expandedRequests[req._id] ? '' : 'line-clamp-1'}`}>
                          &quot; {req.reason} &quot;
                        </span>
                        <ChevronDown 
                          className={`w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-500 transition-transform duration-200 flex-shrink-0 mt-0.5 ${
                            expandedRequests[req._id] ? 'rotate-180' : ''
                          }`} 
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <button
                          onClick={() => resolveOverride(req._id, 'APPROVED')}
                          disabled={resolvingId !== null}
                          className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-lg transition disabled:opacity-50 cursor-pointer shadow-xs"
                        >
                          <Check className="w-3.5 h-3.5" />
                          Approve
                        </button>
                        <button
                          onClick={() => resolveOverride(req._id, 'REJECTED')}
                          disabled={resolvingId !== null}
                          className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 hover:border-rose-300 font-bold text-xs rounded-lg transition disabled:opacity-50 cursor-pointer shadow-xs"
                        >
          ...
          <div className="pt-4 mt-5 border-t border-slate-100 flex items-center justify-between gap-3 select-none">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-rose-600 uppercase tracking-wider">Adjudication Active</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-normal text-right">
              Review overrides live.
            </p>
          </div>
        </div>
      ...
      {/* Roster & Attendance Status Breakdown Card */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
        <h3 className="text-base font-bold text-slate-800 mb-2">Today&apos;s Attendance Status Breakdown</h3>
        <p className="text-xs text-slate-400 mb-5">System-wide status count summary for all active employees today.</p>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: 'Present', count: presentCount, status: 'PRESENT' },
            { label: 'Work From Home', count: wfhCount, status: 'WFH' },
            { label: 'Late Arrival', count: lateCount, status: 'LATE' },
            { label: 'Early Leave', count: earlyCount, status: 'EARLY_LEAVE' },
            { label: 'Paid Sick Leave', count: pslCount, status: 'PAID_SICK_LEAVE' },
            { label: 'Leave Without Pay', count: lwpCount, status: 'LWP' },
          ].map((item) => {
            const config = ATTENDANCE_STATUS_CONFIG[item.status as AttendanceStatus];
            return (
              <div 
                key={item.label} 
                className={`p-4 rounded-xl border text-center flex flex-col justify-center items-center gap-1 hover:scale-[1.02] transition shadow-xs ${
              ...
              >
                ...
                <span className="text-[9px] opacity-60">employees today</span>
              </div>
            );
          })}
        </div>
        ...
        {pendingCount > 0 && (
          <div className="bg-amber-50/50 border border-amber-200/70 p-4 rounded-xl mt-5 flex items-start gap-3">
            <ShieldAlert className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="text-xs font-bold text-amber-800">Pending Roster Statuses Awaiting Input</h4>
              <p className="text-[11px] text-amber-700/90 leading-relaxed mt-1">
                There are <strong>{pendingCount} active employees</strong> with unmarked attendance records for today. 
                Roster tier calculations, compliance sandwich conversions, and balance tallies will not run completely for these records until marked.
              </p>
            </div>
          </div>
        )}
