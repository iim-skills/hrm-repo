...
export default function MonthEndTiersPage() {
  ...
  // New States for Manager & Department filters
  ...
  const fetchManagers = async () => {
    try {
      ...
      console.log('Fetched and sorted managers alphabetically:', mgrs);
      setManagers(mgrs);
    } catch (err) {
      console.error('Error fetching/sorting managers:', err);
    }
  };
  ...
  // Combine filters: Tier + Manager + Department
  const filteredListings = tierListings.filter((item) => {
    ...
    const matchesManager = selectedManager === 'all' || item.employee?.managerId === selectedManager;
    const matchesDepartment = selectedDepartment === 'all' || item.employee?.department === selectedDepartment;
    return matchesTier && matchesManager && matchesDepartment;
  });
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div className="space-y-1 flex-shrink-0">
          <h3 className="text-base font-bold text-slate-800">Month-End &amp; Roster Tiers</h3>
          <p className="text-xs text-slate-400">
            Select a payroll month and tier to automatically view employee roster groupings.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <CustomSelect
            ...
            options={[
              ...
              ...managers.map((mgr) => ({ value: mgr._id, label: mgr.name }))
            ]}
          />
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs min-h-[300px]">
        {loadingTiers ? (
          <div className="py-20 text-center">
            ...
            <p className="text-slate-500 font-semibold text-sm">Syncing with Frozen monthly ledger...</p>
          </div>
          ...
          <div className="py-14 text-center border border-dashed border-slate-200 rounded-xl bg-white">
            ...
            <p className="text-xs text-slate-400 mt-1">There are no employees calculated under Roster Tier {selectedTierFilter} for this month.</p>
          </div>
        ) : (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-0.5">
                              {emp.department || '—'}
                              {emp.managerId && ` • Reports to: ${managerMap.get(emp.managerId) || 'None'}`}
                            </p>
                          </div>
                        </div>
                      </td>
